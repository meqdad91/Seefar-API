...
<div class="bg-indigo-900 text-white rounded-xl shadow-card p-5 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div class="flex items-start gap-3">
        <div class="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0 text-brand-300">
            @include('partials.icon', ['name' => 'users', 'class' => 'w-5 h-5 text-indigo-300'])
        </div>
        <div>
            <h3 class="text-sm font-semibold text-white">What is a Cohort?</h3>
            <p class="text-xs text-indigo-200 mt-1 leading-relaxed max-w-3xl">
                A <strong>Cohort</strong> is a defined group of learners (e.g. intake batch, regional training group, or program track) who are enrolled collectively into courses. Cohorts allow administrators to track progress, completion rates, and active engagement across entire groups rather than individual courses.
            </p>
        </div>
    </div>
    <a href="{{ route('projects.index') }}" class="px-3 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-xs font-semibold text-white transition whitespace-nowrap self-start md:self-auto">
    ...
    @foreach ($cohorts as $c)
        ...
        <div class="bg-white rounded-xl shadow-card border border-slate-200/70 overflow-hidden hover:shadow-md transition">
            ...
            <div class="px-5 py-3 bg-slate-50/50 border-t border-slate-100 text-right">
                <a href="{{ route('users.index', ['cohort_id' => $c['id']]) }}"
                   class="text-xs text-indigo-600 hover:text-indigo-700 font-semibold">View Cohort Members →</a>
            </div>
        </div>
    @endforeach
</div>
